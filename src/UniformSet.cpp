...
{
  gloost::TextureManager::get()->getTextureReference(value.handle);
...
{
  _current_texture_offset = 0;
  addToShader(shader->getShaderHandle(contextId), contextId);
}
...
void
UniformSet::addToShader(gloost::Shader* shader, unsigned int contextId)
{
  addToShader(shader->getShaderHandle(contextId), contextId);
}
