...
PlyLoader::PlyLoader(const std::string& filePath):
  MeshLoader(filePath),
  _inFile(),
  _plyElements(),
  _fileInfo()
...
PlyLoader::readFile(const std::string& filePath)
{
  ...
  std::vector<TriangleFace>& faces     = _mesh->getTriangles();
  faces.clear();
...
//  _mesh->getMaterials()->addMaterial(currentMaterialName, gloost_default_material);
  ...
  if (!_inFile.openAndLoad(filePath))
  {
    ...
    std::cerr << std::endl << "ERROR in PlyLoader::readFile(const std::string& filePath): ";
  ...
  if (strcmp(word.c_str(), "ply"))
  {
    ...
    std::cerr << std::endl << "Message from PlyLoader::readFile(const std::string& filePath): ";
  ...
  for (unsigned int e=0; e!=_plyElements.size(); ++e)
  {
    ...
    {
//      faces = std::vector<TriangleFace>(element.count);
      readFaces(element);
    }
  ...
  _mesh->recalcBoundingBox();
...
#ifndef GLOOST_SYSTEM_DISABLE_OUTPUT_MESSAGES
    unsigned int  memoryConsumption =    _mesh->getVertices().size()  * sizeof(Point3)
    ...
    std::cerr << std::endl << "               Number of vertices:      " << _mesh->getVertices().size();
...
PlyLoader::readVertices(const PlyElement& element )
{
  ...
  for (unsigned int v=0; v!=element.count; ++v)
  {
    ...
    _mesh->getVertices()[v] = Point3(x,y,z);
...
PlyLoader::readPropertyValue(PLY_PROPERTY_TYPE propertyType)
{
  ...
  {
    return (float) _inFile.readChar8();
  }
