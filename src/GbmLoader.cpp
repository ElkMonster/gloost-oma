...
GbmLoader::GbmLoader(const std::string& filePath):
  MeshLoader(filePath)
{
  readFile(filePath);
}
...
/*virtual*/
bool
GbmLoader::readFile(const std::string& filePath)
{
  BinaryFile inFile;
  ...
  if (!inFile.openAndLoad(filePath))
  {
    ...
    std::cerr << std::endl << "ERROR in GbmLoader::readFile(const std::string& filePath): ";
...
//    std::cerr << std::endl << "Message from GbmLoader::readFile(const std::string& filePath): ";
...
//#endif
  ...
  for (unsigned int i=0; i!=numFeatures; ++i)
  {
    ...
    {
      _mesh->getVertices() = std::vector<Point3>(features[i].count);

      for (unsigned int d=0; d!=features[i].count; ++d)
      {
        _mesh->getVertices()[d][0] = inFile.readFloat32();
        _mesh->getVertices()[d][1] = inFile.readFloat32();
        _mesh->getVertices()[d][2] = inFile.readFloat32();
      }
    }
    // normals
    ...
    {
      _mesh->getInterleavedAttributes() = std::vector<float>(features[i].count2);
    ...
    {
      _mesh->getLines() = std::vector<Line>(features[i].count);
    ...
    {
      _mesh->getTriangles() = std::vector<TriangleFace>(features[i].count);
    ...
    {
      _mesh->getQuads() = std::vector<QuadFace>(features[i].count);
