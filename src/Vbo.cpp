...
Vbo::initInContext (unsigned int contextId)
{
  ...
  else
  {
    ...
    interleavedMesh = new Mesh(_mesh, true);
  }
...
//  }
  ...
  if (interleavedMesh->getQuads().size())
  {
    ...
    std::vector<QuadFace>& srcQuads = interleavedMesh->getQuads();
...
//    std::cerr << std::endl << "word: " << interleavedMesh->getPoints().size();
...
//  CheckErrorsGL("befor bind in -> gloost::Vbo::bind(unsigned int supportIndexRanges, unsigned int contextId)");
...
//  CheckErrorsGL("after bind in -> gloost::Vbo::bind(unsigned int supportIndexRanges, unsigned int contextId)");
...
//  CheckErrorsGL("befor glDrawElements in -> gloost::Vbo::draw(unsigned int contextId)");
...
//  CheckErrorsGL("after glDrawElements in -> gloost::Vbo::draw(unsigned int contextId)");
