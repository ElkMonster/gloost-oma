  ...
  glPushAttrib(GL_ALL_ATTRIB_BITS);
  {
...

    {
      glPushMatrix();
      {
        ...
        gloost::drawWireRaster((unsigned int)(grid1_subX*ar), grid1_subY);


        glLineWidth(2.0);
        gloost::drawWireRaster((unsigned int)(grid3_subX*ar), grid3_subY);
      }
      glPopMatrix();
    }
    ...
    for (unsigned int i = _graph->range-_drawRange+offset; i < _graph->range + offset; ++i)
    {
      ...
      float shiftY = gloost::min((float) height, currentValue*heightCoeff);
    ...
    {
      glTranslated(posX,
                   posY + gloost::min( (float) height, meanValue*heightCoeff),
                   0.0f);
    ...
    _textMaker->renderTextLine(posX + width/2,
                               posY +3,
                               gloost::toString(_graph->values->at(max(0, _graph->entryPoint - 1))));
