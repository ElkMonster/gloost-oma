...
bool
Widget::handleMouse(gloost::Mouse* mouse)
{
  ...
  if (inside)
  {
    ...
    /// mouseDown
    if (!getPropBool("_mouseDown") && mouse->getMouseDownEvent(GLOOST_MOUSE_BUTTON1))
    {
      mouseDown();
    }
    ...
    /// mouseUp
    if (mouse->getMouseUpEvent(GLOOST_MOUSE_BUTTON1))
    {
      mouseUp();
    }
