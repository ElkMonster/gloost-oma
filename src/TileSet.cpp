...
TileSet::~TileSet()
{
	gloost::TextureManager::getInstance()->dropReference(_atlasTextureId);
}
                          ...
                          unsigned int numTilesV)
{
  ...
  _atlasTextureId = gloost::TextureManager::getInstance()->createTexture(textureFilePath);

  _numTilesH = numTilesH;
  _numTilesV = numTilesV;

  _atlasTexture = gloost::TextureManager::getInstance()->getTextureReference(_atlasTextureId);
...
{
  // loop animation
  frameNum = (int)gloost::fmod((mathType)frameNum, (mathType)_numFrames);
