...
/*virtual */
void
ZeroMqNode::queue(BinaryBundle* bundle)
{
...
ZeroMqNode::getRecievedBundle()
{
  if (_inQueue.size())
  {
    ...
    BinaryBundle* bundle = _inQueue.front();
    _inQueue.pop_front();
    return bundle;
  }
...
ZeroMqNode::getLastRecievedBundle()
{
  ...
  {
    boost::mutex::scoped_lock lock(_inQueueMutex);
    BinaryBundle* bundle = _inQueue.back();
    _inQueue.pop_back();
    return bundle;
  }
...
ZeroMqNode::sendMessage()
{
  ...
  if (_outQueue.size())
  {
    BinaryBundle* bundle = _outQueue.front();
...
ZeroMqNode::receiveMessage()
{
  ...
  if (inMessage.size())
  {
    BinaryBundle* bundle = new BinaryBundle((unsigned char*)inMessage.data(), inMessage.size());
