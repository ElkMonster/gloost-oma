    ...
    _polygonOffsetUnits(3.0)
{
  ...
  _camera->setAspect((GLfloat)(smap_width)/(GLfloat)(smap_height));
  ...
  _shadowMapTexture = new Texture(smap_width,
                                   smap_height,
                                   GL_TEXTURE_2D,
                                                   ...
                                                   const Camera* cam)
{
  ...
  Matrix newShadowmapMatrix(cam->getModelViewMatrix());
  newShadowmapMatrix.invert();
  newShadowmapMatrix.mult(_camera->getModelViewMatrix());
                                                   ...
                                                   const Point3& sceneMax)
{
  ...
  */
  ...
  const float fov(360.0f*(atan(scene_radius/nearPlaneZ)/gloost::PI));
...
ShadowMapPointLight::applyShadowMapMatrix(const Camera* cam){
  ...
  Matrix smap_matrix(cam->getModelViewMatrix());
  smap_matrix.invert();
  smap_matrix.mult(_camera->getModelViewMatrix());
