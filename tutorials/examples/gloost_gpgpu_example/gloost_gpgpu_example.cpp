...
/**
  ...
  Last change 09.04.24 by Felix Weiﬂig
...
gloost::Shader*     g_processingShader   = 0;
gloost::UniformSet* g_processingUniforms = 0;


/// shader and uniforms for visualsation
gloost::Shader*     g_viewShader   = 0;
gloost::UniformSet* g_viewUniforms = 0;
...
void init()
{
  ...
  for (unsigned int i=0; i!=2; ++i )
  {
    ...
    gloost::Texture* buffer = new gloost::Texture(g_bufferWidth,
                                                  g_bufferHeight,
                                                  16,
                                                  GL_TEXTURE_2D,
                                                  GL_RGBA32F,
  ...
  gloost::Texture* background = new gloost::Texture(g_dataPath + "/background/wcip_screenshot.jpg");
  ...
  g_processingUniforms = new gloost::UniformSet();
  g_viewUniforms       = new gloost::UniformSet();
  g_viewUniforms->set_sampler2D("environment_map", g_texMaster->createTexture("../../data/environments/probe.jpg"));
...
void reloadShaders()
{
  ...
  g_processingShader   = new gloost::Shader(g_dataPath + "/shader/processingShader.vs",
  ...
  g_viewShader     = new gloost::Shader(g_dataPath + "/shader/visualizingShader.vs",
...
void processLiquid()
{
  ...
  glPushMatrix();
  {
    glPushAttrib(GL_ALL_ATTRIB_BITS);
    {
      ...
      g_processingUniforms->set_sampler2D("data_map", g_doubleBufferIds[(int)(!g_currentBuffer)]);
      g_processingUniforms->set_sampler2D("targetTex", g_doubleBufferIds[(int)(g_currentBuffer)]);
      g_processingUniforms->set_sampler2D("objects_map", g_brush->getTransferTextureId());
      ...
      {
        g_processingShader->set();
        g_processingUniforms->applyToShader(g_processingShader->getShaderHandle());
        {
          glPushMatrix();
...
int main(int argc, char *argv[])
{
  ...
  glutCreateWindow("gpu_draw_test");
